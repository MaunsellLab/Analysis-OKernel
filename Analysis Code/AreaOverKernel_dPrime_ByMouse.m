...
V1LumDs = zeros(length(animals),1);

for a = 1:length(animals)
    limits.aniNum = a;
    limits.animal = animals{a};
    U = selectUsingLimits(lumT, limits);
    V1LumDs(a,1) = nanmean([U.noStimDPrime]);
    lumKernel = []; % Init
...
V1GabDs = zeros(length(animals),1);

for a = 1:length(animals)
    limits.aniNum = a;
    limits.animal = animals{a};
    U = selectUsingLimits(gabT, limits);
    V1GabDs(a,1) = nanmean([U.noStimDPrime]);
    gabKernel = []; % Init
    % get all Profiles from this mouse
    for i = 1:height(U)
        load(strcat('/Users/jacksoncone/Dropbox/PostDoctoral Projects/!Experiments/Colliculus/MasterFiles/',...
...
SCLumDs = zeros(length(animals),1);

for a = 1:length(animals)
    limits.aniNum = a;
    limits.animal = animals{a};
    U = selectUsingLimits(lumT, limits);
    SCLumDs(a,1) = nanmean([U.noStimDPrime]);
    lumKernel = []; % Init
    % get all Profiles from this mouse
    for i = 1:height(U)
        load(strcat('/Users/jacksoncone/Dropbox/PostDoctoral Projects/!Experiments/Colliculus/MasterFiles/',...
...
SCGabDs = zeros(length(animals),1);

for a = 1:length(animals)
    limits.aniNum = a;
    limits.animal = animals{a};
    U = selectUsingLimits(gabT, limits);
    SCGabDs(a,1) = nanmean([U.noStimDPrime]);
    gabKernel = []; % Init
    % get all Profiles from this mouse
    for i = 1:height(U)
        load(strcat('/Users/jacksoncone/Dropbox/PostDoctoral Projects/!Experiments/Colliculus/MasterFiles/',...
...
[rho1,p1] = corr(V1GabAOKs, V1GabDs);
[rho2,p2] = corr(SCGabAOKs, SCGabDs);
[rho3,p3] = corr(V1LumAOKs, V1LumDs);
[rho4,p4] = corr(SCLumAOKs, SCLumDs);

% Fit Linear Function
f1 = fitlm(V1GabAOKs, V1GabDs);
f2 = fitlm(SCGabAOKs, SCGabDs);
f3 = fitlm(V1LumAOKs, V1LumDs);
f4 = fitlm(SCLumAOKs, SCLumDs);
...
subplot(2,2,1);
hold on;
scatter(V1GabAOKs, V1GabDs, 50, 'black', 'filled');
...
subplot(2,2,2);
hold on;
scatter(V1LumAOKs, V1LumDs, 50, 'black', 'filled');
...
subplot(2,2,3);
hold on;
scatter(SCGabAOKs, SCGabDs, 50, 'black', 'filled');
...
subplot(2,2,4);
hold on;
scatter(SCLumAOKs, SCLumDs, 50, 'black', 'filled');

...
function [x y F_even F_odd] = gaborFig(varargin)
    ...
    % Generate gabor
    % Even Symmetric
    F_even = 127.5+exp(-.5*(x_theta.^2/p.Results.Sigma^2+y_theta.^2/p.Results.Sigma^2)).*cos(2*pi/p.Results.lambda*x_theta);
    % Odd-Symmetric
    F_odd = exp(-.5*(x_theta.^2/p.Results.Sigma^2+y_theta.^2/p.Results.Sigma^2)).*sin(2*pi/p.Results.lambda*x_theta);

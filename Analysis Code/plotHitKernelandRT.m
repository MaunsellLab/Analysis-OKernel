function plotHitKernelandRT(stimulus, dp_cut, nSigma, dp_cut_at0)
%stimulus = 'Lum';
%stimulus = 'Gabor';
%stimulus = 'Offset';
%stimulus = 'FA';

% Function takes a stimulus condition and plots the hit and miss 
% kernels for comparisons across V1 and SC
% 'Lum'
% 'Gabor'
% 'Offset'
% 'FA'

% dp_cut: if true, it drops sessions that have delta d-prime > mu +nSigma*sigma

% dp_cut_at0: if true, drops any session with a delta d' > 0. Overrides dp_cut
if dp_cut_at0 == 1
    dp_cut = 0;
end
...
%% Stimulus Aligned Hit Kernels
% Bootstrap V1
v1BootKernel = bootstrp(250, @mean, v1Kernel_hit);
...
% Bootstrap SC
scBootKernel = bootstrp(250, @mean, scKernel_hit);
...
subplot(2,2,1);
yyaxis left
plot([0, bins], [0.50, 0.50], 'Color', 'k', 'LineStyle','--');
hold on;
...
plotTitle = sprintf('V1 Hit Kernel (n=%d)', nHitsV1);
title(plotTitle);
hold off;
...
%% Stimulus Aligned Miss Kernels
% Bootstrap V1
v1BootKernel = bootstrp(250, @mean, v1Kernel_miss);
v1PCs = prctile(v1BootKernel, [15.9, 50, 84.1]);              % +/- 1 SEM
v1PCMeans = mean(v1PCs, 2);
CIs = zeros(3, size(v1BootKernel, 2));
for c = 1:3
...
% Bootstrap SC
scBootKernel = bootstrp(250, @mean, scKernel_miss);
scPCs = prctile(scBootKernel, [15.9, 50, 84.1]);              % +/- 1 SEM
scPCMeans = mean(scPCs, 2);
CIs = zeros(3, size(scBootKernel, 2));
for c = 1:3
...
%% Add Miss Kernels to Existing Plot
% V1
subplot(2,2,3);
hold on;
plot([0, bins], [0.50, 0.50], 'Color', 'k', 'LineStyle','--');
...
ax.XAxis.MinorTickValues = [0 100 200 300 500 600 700 800];
xlabel('Time Relative to Stimulus');
plotTitle = sprintf('V1 Miss Kernel (n=%d)', nMissV1);
title(plotTitle);
hold off;
...
ax.XAxis.MinorTickValues = [0 100 200 300 500 600 700 800];
xlabel('Time Relative to Stimulus');
plotTitle = sprintf('SC Miss Kernel (n=%d)', nMissSC);
title(plotTitle);
hold off;

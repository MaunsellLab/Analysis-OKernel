function areaOverKernel_GrandAv(brainArea, bootSamps, analysisStartMS, analysisDurMS);
...
gabAOK = zeros(1,1);
lumAOK = zeros(1, 1);
lumAOK_95CI  = zeros(1,2);
gabAOK_95CI  = zeros(1,2);
...
gabAOK(1,1) = sum(mean(gabKernel(:,analysisStartBin:analysisEndBin)));
lumAOK(1,1) = sum(mean(lumKernel(:,analysisStartBin:analysisEndBin)));
...
lumAOK_95CI(1,:) = quantile(lumBootsAOK,[0.025 0.975]);
gabAOK_95CI(1,:) = quantile(gabBootsAOK,[0.025 0.975]);

%% Make Scatter Plot of Results
figure;
hold on;
axis square;
scatter(lumAOK, gabAOK, 100, 'black','filled');
% Plot 95% CIs
plot([lumAOK_95CI(1,1)  lumAOK_95CI(1,2)], [gabAOK gabAOK], 'LineStyle','-', 'LineWidth',2, 'Color', 'k');
plot([lumAOK lumAOK], [gabAOK_95CI(1,1) gabAOK_95CI(1,2)], 'LineStyle','-', 'LineWidth',2, 'Color', 'k');
